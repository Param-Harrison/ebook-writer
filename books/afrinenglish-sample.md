...
```mermaid
graph TD
  A[Present Simple] --> B[I am / You are]
  A --> C[He/She is]
  A --> D[We/They are]
  B --> E[How are you?]
  C --> F[How is she?]
...
sequenceDiagram
  participant You
  participant Friend
  You->>Friend: Are you free?
  Friend->>You: Yes, what's the plan?
  You->>Friend: Let's go to the cinema
  Friend->>You: Great idea!
```
...
sequenceDiagram
  ...
  participant Helper
  participant Police
  You->>Helper: I need help
  Helper->>You: What's wrong?
  You->>Helper: I lost my wallet
  Helper->>Police: Call for assistance
  Police->>You: We'll help you
```
...
```mermaid
graph TD
  A[Simple Sentence] --> B[Add Connector]
  B --> C[Add Detail]
  C --> D[Complex Sentence]
  D --> E[More Natural Speech]
```
...
```mermaid
graph TD
  A[Start Conversation] --> B[Use Greetings]
  B --> C[Ask Questions]
